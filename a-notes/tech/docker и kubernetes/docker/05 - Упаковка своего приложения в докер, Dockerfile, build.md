...
- Чтобы запустить шаги clean, потом package, можно 2хЛКМ по каждому шагу

## Подготовка Dockerfile

Чтобы докер понимал, как это делать, создаем в корне приложения (на одном уровне с .pom) файл `Dockerfile` (без расширения и с большой буквы) с таким содержимым:
...
* `FROM`
  * задает образ-основу для нашего собственного образа
  * через гугл на hub docker jdk ищем актуальную версию
  * например, openjdk теперь считается deprecated
* `WORKDIR`
  * задает "рабочую" директорию внутри образа, относительно которой будут ориентироваться команды COPY, CMD (и другие, вроде RUN, ADD, ENTRYPOINT). 
  * Если директории не существует, она создается автоматически
* `COPY` 
  * копирует *откуда* - *куда*
    * Откуда в данном случае это папка с программой на нашем компьютере, а . - это директория внутри будущего образа, которую мы задали через WORKDIR (/app/ в данном случае)
* `EXPOSE` 
  * указывает, какой порт слушает работающее внутри контейнера приложение.
    * Можно указывать EXPOSE несколько раз, если портов больше одного. 
    * Эта команда служит для информационных целей, чтобы  пользователь образа через команду `docker inspect SomeImageName` мог узнать о портах
    * Т.е. технически можно вообще не указывать expose, это только информационный хинт, так сказать, соответственно, он должен быть такой же, какой указан в yaml приложения
* `CMD` 
  * задает команду, которая будет выполняться при запуске контейнера. 
  * Здесь мы указываем просто имя нашей программы, без полного пути, опять же за счет того, что задали WORKDIR.
  * CMD фактически может быть только одна, потому что если указать их несколько, то выполнится только последняя
...
- `-t` - задаем человекочитаемое имя образа
- `.` - это директория с файлом Dockerfile (в данном случае текущая).

После сборки:

- в docker desktop появляется этот образ и его можно запустить как обычно.
- Образ не появляется как отдельный файл на диске, он лежит где-то в кишках докера, его можно увидеть в разделе containers в docker desktop, либо командой docker images
- Но его можно экспортировать, чтобы он реально стал отдельным файлом. И еще его можно публиковать. Но это отдельный разговор.
