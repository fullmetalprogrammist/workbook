...
- Способы установки
  - Локально на свой комп через установочник
  - Поднять контейнер и пользоваться через браузер

## Установка через контейнер

```
docker run -p 8080:80 -e PGADMIN_DEFAULT_EMAIL=admin@admin.com -e PGADMIN_DEFAULT_PASSWORD=admin --name pgadmin dpage/pgadmin4
```

- `dpage/pgadmin4` - имя образа, если его нет, то сначала автоматически скачается
- после запуска контейнера, pgAdmin доступен в браузере по адресу http://localhost:8080
  - для авторизации используем креды, которые задали при запуске контейнера - почту и пароль
