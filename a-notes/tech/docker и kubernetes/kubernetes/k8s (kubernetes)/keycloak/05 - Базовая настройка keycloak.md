...
- yaml с официального сайта, немного подправленный под следующие условия
  - доступ к кластеру сделан через gateway api, а keycloak используется исключительно внутри кластера, поэтому из конфига убран ingress-ресурс
...
- применять yaml командой с ключом `-n`, чтобы все ресурсы попали в неймспейс keycloak
  - при этом неймспейс ресурсов в самом манифесте мы не указываем

```
kubectl apply -f "путь\до\keycloak.yaml" -n keycloak
```

- прокинуть порт, чтобы админка была доступна из браузера
