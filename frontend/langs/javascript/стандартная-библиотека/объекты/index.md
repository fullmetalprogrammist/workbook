...
- Способы создания объекта
  ...
  - `Object.create()` - редкие кейсы, вроде
    - Создание объекта без прототипа
    - Создание объекта с явным указанием прототипа
  - Функция-конструктор (через оператор `new`)
    - Обычно когда речь идет о создании экземпляра класса
...
- Механизм поиска свойства
  - сначала у себя > потом в прототипе и далее по цепочке прототипов вверх
...
- Проверять только собственные свойства
  - Метод `Object.hasOwn(obj, 'foobar')`
- Проверять у себя и в прототипе
  - оператор `in`
    - `if ('foobar' in obj)`
...
- Удалить свойство из объекта
  - Оператор `delete`
    - `delete user.lastname` или `delete user["lastname"]`
...
- Части объекта можно получить как массивы и обойти:
  ...
  - Не путать с этими же методами у Map, Set - там они возвращают iterable-объекты.
