...
Движок может интерпретировать оператор "три точки" `...` двумя способами, в зависимости от контекста:

* `spread` - в местах, где надо *указать* значение. Например, создание массивов (указать элементы), объектов (указать поля).
* `rest` - в местах, где надо *принять* значение. Например, параметры функций, деструктурирующее присваивание.

# ... как spread

В местах, где надо *указать* значение, `...` это spread.
...
`spread` - это "разбить". Как взять массив, объект, и разбить его об пол на отдельные куски.
